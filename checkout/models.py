...
class Category(models.Model):
    ...
    name = models.CharField(
        null=False, blank=False, max_length=254, choices=CATEGORY_OPTIONS)
    friendly_name = models.CharField(
        default="default", max_length=254, blank=True)
...
class Order(models.Model):
    ...
    postcode = models.CharField(default="", max_length=20, blank=True)
    town_or_city = models.CharField(
        default="", max_length=40, null=False, blank=False)
    street_address1 = models.CharField(
        default="", max_length=80, null=False, blank=False)
    street_address2 = models.CharField(default="", max_length=80, blank=True)
    county = models.CharField(default="", max_length=80, blank=True)
...
class OrderLineItem(models.Model):
    ...
    friendly_name = models.CharField(default="", max_length=254)
    ...
    variations = models.DecimalField(
        default=1, max_digits=3, decimal_places=2, choices=VARIATION_CHOICES)
    user_description = models.TextField(default="", null=False, blank=False)
    fast_delivery = models.BooleanField(
        default=False, null=True, blank=True, choices=DELIVERY_CHOICES)
    is_complete = models.BooleanField(default=False, null=True, blank=True)
    display_in_portfolio = models.BooleanField(
        default=False, null=True, blank=True, choices=DISPLAY_OPTIONS)
    ...
    def save(self, *args, **kwargs):
        ...
        delivery = 1.00
        if self.fast_delivery is True:
            delivery = Decimal(round(settings.FAST_DELIVERY_CHARGE, 2))
        self.lineitem_total = round(
            Decimal(
                self.category.price) * Decimal(self.complexity) * Decimal(
                    self.variations) * Decimal(delivery), 2)
        super().save(*args, **kwargs)
